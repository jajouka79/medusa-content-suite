...
namespace Codecourse\FieldTypes;

echo "-Codecourse\FieldTypes-Codecourse\FieldTypes-Codecourse\FieldTypes";
/**
...
function jt_cmb2_get_address_field( $metakey, $post_id = 0 ) {
	...
	$address .= '<p><strongAddress:</strong> ' . esc_html( $address['address1'] ) . '</p>';
	if ( $address['address2'] ) {
		$address .= '<p>' . esc_html( $address['address2'] ) . '</p>';
	}
	$address .= '<p><strong>City:</strong> ' . esc_html( $address['city'] ) . '</p>';
	$address .= '<p><strong>County:</strong> ' . esc_html( $address['county'] ) . '</p>';
	$address .= '<p><strong>Postcode:</strong> ' . esc_html( $address['postcode'] ) . '</p>';
...
function jt_cmb2_render_address_field_callback( $field, $value, $object_id, $object_type, $field_type_object ) {
	...
	$select_text = esc_html( $field_type_object->_text( 'address_select_county_text', 'Select a County' ) );
    ...
    foreach ( $english_counties as $abrev => $county ) {
        $county_options .= '<option value="'. $county. '" '. selected( $value['county'], $county ) . '>'. $county. '</option>';
    }
	...
	<div><p><label for="<?php echo $field_type_object->_id( '_address_1' ); ?>"><?php echo esc_html( $field_type_object->_text( 'address_address_1_text', 'Address 1' ) ); ?></label></p>
		<?php echo $field_type_object->input( array(
			'name'  => $field_type_object->_name( '[address1]' ),
	...
	<div><p><label for="<?php echo $field_type_object->_id( '_address_2' ); ?>'"><?php echo esc_html( $field_type_object->_text( 'address_address_2_text', 'Address 2' ) ); ?></label></p>
		<?php echo $field_type_object->input( array(
			'name'  => $field_type_object->_name( '[address2]' ),
	...
	<div class="alignleft"><p><label for="<?php echo $field_type_object->_id( '_county' ); ?>'"><?php echo esc_html( $field_type_object->_text( 'address_county_text', 'County' ) ); ?></label></p>
		<?php echo $field_type_object->select( array(
			'name'    => $field_type_object->_name( '[county]' ),
			'id'      => $field_type_object->_id( '_county' ),
			'options' => $county_options,
		) ); ?>
	</div>
	<div class="alignleft"><p><label for="<?php echo $field_type_object->_id( '_postcode' ); ?>'"><?php echo esc_html( $field_type_object->_text( 'address_postcode_text', 'Postcode' ) ); ?></label></p>
		<?php echo $field_type_object->input( array(
			'class' => 'cmb_text_small',
			'name'  => $field_type_object->_name( '[postcode]' ),
			'id'    => $field_type_object->_id( '_postcode' ),
...
#include ('fields/uk-address-field-type.php');
